...
final class TImpalaQueryOptions {
  ...
  const HBASE_CACHING = 14;
  const HBASE_CACHE_BLOCKS = 15;
  const PARQUET_FILE_SIZE = 16;
  const EXPLAIN_LEVEL = 17;
  const SYNC_DDL = 18;
  const REQUEST_POOL = 19;
  const V_CPU_CORES = 20;
  const RESERVATION_REQUEST_TIMEOUT = 21;
  const DISABLE_CACHED_READS = 22;
  const DISABLE_OUTERMOST_TOPN = 23;
  const RM_INITIAL_MEM = 24;
  const QUERY_TIMEOUT_S = 25;
  const MAX_BLOCK_MGR_MEMORY = 26;
  const APPX_COUNT_DISTINCT = 27;
  const DISABLE_UNSAFE_SPILLS = 28;
  ...
  public function read($input)
  {
    ...
    while (true)
    {
      ...
      switch ($fid)
      {
        ...
        case 2:
          if ($ftype == TType::STRUCT) {
            $this->summary = new \impala\TExecSummary();
